...
export default {
  ...
  methods: {
    ...
    photoOnClick(filename, id, slug) {
      ...
      let photoViewRoute =
        route().current() == "photos.trash" ? "photos.trash.show" : "photos.show";

      const nextURL = route(photoViewRoute, {
        id: id,
        slug: slug,
      });
    ...
    genFetMoreUrl() {
      ...
      return route("index.fetch_more") + new URL(this.photos.next_page_url).search;
    },
  },
  unmounted() {
    this.$nextTick(() => {
